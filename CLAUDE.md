...
| 「ダッシュボード更新して」 | `python3 generate_dashboard.py` を実行(carry/spread/ranking/intelの4データを再取得しHTMLに埋め込む) |
...
- `.github/workflows/refresh-dashboard.yml` が毎時 `generate_dashboard.py` を実行し
  ダッシュボードHTMLを自動コミットする。この経路はLLM呼び出し不可の純Pythonのみ
  (`risk_manager.py` / `market_intel.py` はこの制約を満たす設計)
